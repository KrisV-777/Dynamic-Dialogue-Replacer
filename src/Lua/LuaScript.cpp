...
namespace DDR
{
    LuaScript::LuaScript(const YAML::Node& a_node) :
    ...
    bool LuaScript::CanApplyReplacement(RE::TESObjectREFR* a_speaker, RE::TESObjectREFR* a_target, Type a_type) const
    {
        if (_type != Type::Any && _type != a_type)
            return false;
        if (_speakerId != 0) {
